...
def restore_tech_names(text: str) -> str:
    ...
    out = re.sub(r"(?i)\bK\s*8\s*s\b", "K8s", out)
    out = re.sub(r"(?i)\bJ\s*2\s*EE\b", "J2EE", out)
    out = re.sub(r"(?i)\bDB\s*2\b", "DB2", out)
    out = re.sub(r"(?i)\b5\s*G\b", "5G", out)
    out = re.sub(r"(?i)\bv\s*(\d+)\b", r"v\1", out)
    return out
