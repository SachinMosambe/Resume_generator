...
def find_libreoffice() -> str | None:
    """Locate LibreOffice / soffice binary used for .doc → .docx conversion."""
    for candidate in ("soffice", "libreoffice"):
        path = shutil.which(candidate)
        if path:
            return path
    # Common absolute paths on Ubuntu/Debian.
    for path in (
        "/usr/bin/soffice",
        "/usr/bin/libreoffice",
...
def convert_doc_to_docx(source: Path, output_dir: Path | None = None) -> Path:
    ...
    if not converted.exists():
        ...
        if candidates:
            ...
            logger.error("doc_conversion_failed", extra={"stderr": stderr[:500]})
            raise DocConversionError(
                "Could not convert .doc to .docx. "
                + (stderr[:300] if stderr else "LibreOffice produced no output file.")
            )

    logger.info("Converted .doc → .docx: %s", converted)
    return converted
